...
// 		return from
// 	}
// 	fmt.Println("salkjlsakjdsajlksdsajlsakl")
...
func PushModel(ctx context.Context, name string, regOpts *RegistryOptions, fn func(ProgressResponse)) error {
	mp := ParseModelPath(name)
	fn(ProgressResponse{Status: "retrieving manifest"})
	...
	manifest, _, err := GetManifest(mp)
	if err != nil {
		fn(ProgressResponse{Status: "couldn't retrieve manifest"})
		return err
	}
	...
	fn(ProgressResponse{Status: "pushing manifest"})
	...
	fn(ProgressResponse{Status: "success"})
...
func PullModel(ctx context.Context, name string, regOpts *RegistryOptions, fn func(ProgressResponse)) error {
	...
	fn(ProgressResponse{Status: "pulling manifest"})
	...
	fn(ProgressResponse{Status: "verifying sha256 digest"})
	for _, layer := range layers {
	...
	fn(ProgressResponse{Status: "writing manifest"})
	...
	if noprune == "" {
		fn(ProgressResponse{Status: "removing any unused layers"})
		err = deleteUnusedLayers(nil, deleteMap, false)
		if err != nil {
			return err
		}
